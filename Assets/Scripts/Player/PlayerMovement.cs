...
public class PlayerMovement : NetworkBehaviour
{
	...
	[Networked, Tooltip("Is the current player the king?")]
	public bool IsKing { get; set; }
	
	[Networked, Tooltip("Whick team is current player in?")]
	...
	//이미 획득한 정보(UI 표시 및 중복 방지용)
	public List<GameManager.Infos> GotInfos = new();

	//현재 위치(정보 전달용)
	public LocationZone Location;
	...
	void OnDeadChanged()
	{
		...
		if (GameManager.State.ActiveState is PlayStateBehaviour)
		{
	...
	public override void FixedUpdateNetwork()
	{
		...
		bool canKill = false, canUse = false;
		...
		for (int i = 0; i < lagHit; i++)
		{
			if (lagCompensatedHits[i].Hitbox is Hitbox hb)
			{
				...
				if (!hb.transform.IsChildOf(transform) && hb.gameObject.layer == _playerRadiusLayer && hb.GetComponentInParent<PlayerObject>() is PlayerObject player)
				{
	...
	public void TryKill()
	{
		...
		if (actionPerformed || !KillTimer.ExpiredOrNotRunning(Runner))
			return;

		for (int i = 0; i < nearbyPlayers.Count; i++)
		{
			if (nearbyPlayers[i].Controller.IsDead == false)
			{
