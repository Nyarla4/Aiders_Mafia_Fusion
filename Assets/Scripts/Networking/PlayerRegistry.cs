...
public class PlayerRegistry : NetworkBehaviour, INetworkRunnerCallbacks
{
	...
	public static List<PlayerObject> OtherPlayers(PlayerRef pRef)
    {
		List<PlayerObject> result = new();
        foreach (var pObj in Instance.ObjectByRef)
        {
			if(pObj.Key != pRef)
            {
				result.Add(pObj.Value);
			}
        }
		return result;
    }
	...
	public static PlayerObject GetRandomWhere(System.Predicate<PlayerObject> match)
	{
		...
		byte index = (byte)Random.Range(0, players.Count);
		byte i = 0;
		foreach (var kvp in players)
		{
			PlayerObject pObj = kvp;
			if (pObj != null)
			{
				if (i == index) return pObj;
				i++;
